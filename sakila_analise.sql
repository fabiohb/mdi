...
select 'hour dim', count(*) from (select distinct date(rental_date), date_format(rental_date, '%H') from rental) fact
;
select 'date dim', count(*) from (select distinct date(rental_date) from rental) fact
;
select 'base table', count(*) from rental
union all
select 'fact table1', count(*) 
  from (
       select  r.rental_id as hash, 
               date(rental_date) as rental_date, 
               DATE_FORMAT(rental_date,  '%H') as rental_hour,  
               film_id, store_id, r.staff_id, r.customer_id, 
               datediff(now(), rental_date) as age_rental,  
               datediff(now(), return_date) as age_return, 
...
select  distinct date(rental_date) as date , year(rental_date) as year, month(rental_date) as month, day(rental_date) as day from rental
;