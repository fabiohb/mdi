...
DROP FUNCTION `sakila`.`rental_hash`
;
...
hash int,
processed tinyint(1) /* 0 - Nao processado, 1 - Processando, 2 - Processado */
)
;
...
select rental_hash(date(r.rental_date), i.film_id, i.store_id, r.staff_id, r.customer_id) as hash, 0 as processed
from rental r left join inventory i
on (r.inventory_id = i.inventory_id)
;
...
select customer_id, sum(amount) extra_amount 
  from payment 
...
group by customer_id
order by customer_idfact_rental
;
